...
public class Enigma {
	 ...
	 * 
	 * @param rotors
	 *            The set of rotors to be used in the order they should be used.
	 * @param rotorPositions
	 *            The starting positions of the rotors. This should be the same
	...
	public void rotateRotors(int charPos) {
		for (int i = 0; i < rotors.length; i++) {
			// This is the number of rotations the smallest rotor has to make
			// for one rotation here.
			int turns = (int) Math.pow(26, i);

			// How many rotations has the smallest rotor made since we last
			// turned?
	...
	public static void main(String[] args) {
		...
		if (args.length < 1) {
			System.out.println("Format: java Enigma [ciphertext/plaintext]");
		} else {
			Rotor[] rotors = new Rotor[] { Rotor.ROTOR_I, Rotor.ROTOR_II, Rotor.ROTOR_III, Rotor.ROTOR_IV };
			char[] startPositions = new char[] { 'A', 'A', 'A', 'A' };
			...
			String plaintext = enigma.execute(args[0]);
